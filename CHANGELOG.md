# Changelog

## 0.1.5
- Incluído endereço de **manifest** (GitHub Releases) no README.
- `module.json`: adicionados campos `manifest` e `download` apontando para a release `v0.1.5`.

## 0.1.4
- Preparação para publicação no GitHub: metadata `url`/`bugs` no `module.json`.
- Adicionado README e CHANGELOG.

## 0.1.3
- Extempore agora registra a condição em `customConditions` para exibir **descrição** no token (igual condições nativas).

## 0.1.2
- Ajustes internos de parsing do chat (sem impacto final).

## 0.1.1
- Ajustes internos de parsing do chat (sem impacto final).

## 0.1.0
- Versão inicial: criar/remover efeitos extemporâneos a partir do chat.
